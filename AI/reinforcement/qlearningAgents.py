...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for actions in self.getLegalActions(state):
          if bestVal is self.getQValue(state, actions):
            action = actions
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for key in self.featExtractor.getFeatures(state, action).keys():
          self.getWeights()[key] = self.getWeights()[key] + self.alpha * difference * self.featExtractor.getFeatures(state, action)[key]
        # util.raiseNotDefined()
